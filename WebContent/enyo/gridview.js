...
enyo.kind({
    ...
    components: [
        {kind: "Scroller", classes: "scroller-sample-scroller enyo-fit", components: [
            {
                name: "gridview",
                kind: "shenru.GridFlyWeightRepeater",
                onSetupItem: "setupItem",
                count: 110,
                rowCount : 6,
                components: [
                    {name: "name",ontap:"clicker",  style: "width:100px;  height:100px; background:#00ff00; margin:10px;", content: "哈哈"}
                ]
            }
        ]}
    ],
    ...
    clicker : function(inSender, inEvent){
        alert("item clicker" + inEvent.index);
    },
...
//自定义控件
enyo.kind({
    name: "shenru.GridFlyWeightRepeater",
    published: {
...
//            this.clientStyleChanged();
    ...
    setupItem: function (inIndex) {
        this.doSetupItem({index: inIndex, selected: false});
    },
    //* Renders the list.
    generateChildHtml: enyo.inherit(function (sup) {
        return function () {
            ...
            for (var i = 0, r = 0; i < this.count; i++) {
                ...
                //<div style="border:1px solid black;margin:10px; display:table;">
                //</div>
                ...
                var sh = sup.apply(this, arguments);
                if ((i % this.rowCount == 0)) {
                    ...
                    if (i == this.count - 1) {
                        ...
                        var rowDiv = '<div style="border:1px solid black;display:table;">';
                        var rowDiv2 = '</div>';
                        h += rowDiv;
                        h += sh;
                        h += rowDiv2;
